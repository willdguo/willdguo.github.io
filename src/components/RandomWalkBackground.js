...
export default function RandomWalkBackground() {
  ...
  return <><svg className="random-walk-layer" onClick={event => launch(event.clientX, event.clientY)} aria-hidden="true">{walkers.flatMap(walk => walk.path.map((segment, index) => <line className="draw-line" key={`${walk.id}-${index}`} x1={segment.start[0]} y1={segment.start[1]} x2={segment.end[0]} y2={segment.end[1]} stroke={colorFor(walk.id)} strokeDasharray={segment.length} strokeDashoffset={segment.length} strokeWidth="1.5" />))}</svg><button className="walk-button" onClick={() => launch(bounds.width * .72, bounds.height * .62)} aria-label="Launch a random walk" title="Launch a random walk">↝</button></>;
}
